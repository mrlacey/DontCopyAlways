...
namespace DontCopyAlways
{
	public class SponsorRequestHelper
	{
		...
		private static async Task ShowThanksForSponsorshipMessageAsync()
		{
			...
			await OutputPane.Instance.WriteAsync("please raise an issue at https://github.com/mrlacey/DontCopyAlways/issues");
		...
		private static async Task ShowPromptForSponsorshipAsync()
		{
			...
			await OutputPane.Instance.WriteAsync("leave a positive review at https://marketplace.visualstudio.com/items?itemName=MattLaceyLtd.DontCopyAlways&ssr=false#review-details");
			await OutputPane.Instance.WriteAsync("");
			await OutputPane.Instance.WriteAsync("********************************************************************************************************");
		}
	}
